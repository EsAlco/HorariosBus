    ...
    func chargedNetworking() {
        NetworkingProvider.shared.getAllStopsTrain { allStopsTrainResponse in
            ...
            for feature in allStopsTrainResponse.features{
                locations.append(contentsOf: [Location(nameStop: feature.attributes?.nameStop ?? "", numberStop: feature.attributes?.numberStop ?? "", tariffZoneStop: feature.attributes?.tariffZoneStop ?? "", linesStop: feature.attributes?.linesStop ?? "", typeOfTransport: "Train", xGeometryStop: feature.geometry?.xGeometryStop ?? 0.0, yGeometryStop: feature.geometry?.yGeometryStop ?? 0.0)])
            }
        } failure: { error in
            self.isCharged.toggle()
