...
struct DetailStopTrainView: View {
    ...
    func chargedNetworking() {
        NetworkingProvider.shared.getStopTrain(numberLine: numberStop) { stopTrainResponse in
            ...
            for feature in stopTrainResponse.features {
                nameStop = feature.attributes?.nameStop ?? ""
                numberStop = feature.attributes?.numberStop ?? ""
                tariffZoneStop = feature.attributes?.tariffZoneStop ?? ""
                linesStop = feature.attributes?.linesStop ?? ""
                typeTransport = "Train"
            }
        } failure: { error in
            self.isCharged.toggle()
